...
const SearchPage =()=> {
  ...
  const handleSearch = () => {
    ...
    const searchnum=1;
    const { data } = useGoogleSearch(query,searchnum);
    console.log(data);
  ...
  const clickNum2 = () =>{
    navigation (`/SearchPage2?query=${encodeURIComponent(searchText || transcribedSpeech)}`);
  }
  const clickNum3 = () =>{
    navigation (`/SearchPage3?query=${encodeURIComponent(searchText || transcribedSpeech)}`);
  }
  const clickNum4 = () =>{
    navigation (`/SearchPage4?query=${encodeURIComponent(searchText || transcribedSpeech)}`);
  }
  const clickNum5 = () =>{
    navigation (`/SearchPage5?query=${encodeURIComponent(searchText || transcribedSpeech)}`);
  }
  const clickNum6 = () =>{
    navigation (`/SearchPage6?query=${encodeURIComponent(searchText || transcribedSpeech)}`);
  }
  const clickNum7 = () =>{
    navigation (`/SearchPage7?query=${encodeURIComponent(searchText || transcribedSpeech)}`);
  }
  const clickNum8 = () =>{
    navigation (`/SearchPage8?query=${encodeURIComponent(searchText || transcribedSpeech)}`);
  }
  const clickNum9 = () =>{
    navigation (`/SearchPage9?query=${encodeURIComponent(searchText || transcribedSpeech)}`);
  }
  const clickNum10 = () =>{
    navigation (`/SearchPage10?query=${encodeURIComponent(searchText || transcribedSpeech)}`);
  }
  ...
  return (
    ...
    <div style={{ width: '180px', marginLeft:'20px' }}><Link to='/SearchComponent'><img src={logoImage} alt="Logo" style={{ maxWidth: '100%', maxHeight: '100%',}} /></Link></div>
      ...
      >
          ...
          <div className="searchPage_options" style={{display: 'flex'}}>
            <div className="searchPage_optionsLeft" style={{display: 'flex', marginLeft:'235px', alignItems: 'center',minWidth: '26vw',justifyContent: 'space-between',fontFamily: 'Arial', fontSize: '16px', color: '#000000'}}>
              <div className="searchPage_option" style={{display: 'flex',}}>
                <SearchIcon style={{color:'blue',fontSize:'16px'}}/>
                <a href="/SearchPage" style={{color:'blue',}}>All</a>
              </div>
              <div className="searchPage_option" style={{display: 'flex',}}>
              ...
              <div className="searchPage_option" style={{display: 'flex',}}>
                <ImageIcon style={{color:'#aba8ad',fontSize:'16px'}}/>
                <a onClick={()=>navigation(`/SearchPageImage?query=${encodeURIComponent(searchText || transcribedSpeech)}`)} style={{color:'#aba8ad',textDecoration:'underline'}}>Images</a>
              </div>
              <div className="searchPage_option" style={{display: 'flex',}}>
        ...
        <div className="searchPage__results">
          ...
          {data?.items.map((item) => (
            ...
            <div className="flex flex-col items-center py-14 max-w-[700px]" style={{marginLeft:'190px',marginTop:'0px',marginBottom:'30px'}}>
            ...
            <div style={{display:'flex', marginLeft:'40px'}}>
                <div onClick={()=>clickNum1()} style={{textDecoration:'underline',color:'blue',marginRight:'10px',fontSize:'12px'}}>1</div>
